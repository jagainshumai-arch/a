...
- **AI×教育のパーソナライズ**: 学習スタイル適応型チューター（下記PACE等）

## 注目論文: PACE（パーソナライズ型AI家庭教師）

### 論文情報
- **タイトル**: "One Size doesn't Fit All: A Personalized Conversational Tutoring Agent for Mathematics Instruction"
- **出典**: arXiv:2502.12633（2025年2月、北京理工大学）

### 概要
LLMを使った教育AIの多くは「全員に同じ教え方」をする問題がある。PACEは**生徒一人ひとりの学習スタイルに合わせた数学指導**を実現するエージェント。

### PACEの3ステップ構造
1. **生徒のペルソナ → 学習スタイルをシミュレート**
   - Felder-Silverman学習スタイルモデル（4軸）を採用
   - Active/Reflective、Sensing/Intuitive、Visual/Verbal、Sequential/Global
2. **学習スタイルに合った指導戦略を設計**
   - 性格特性を直接使わず「学習スタイル」に一度変換 → 汎化性が高い
3. **ソクラテス式対話で深い思考を引き出す**
   - 答えを与えず問いかける → 思考力・数学への興味を促進

### 技術的に面白いポイント
- **LLM-to-LLM対話で合成データ生成**: GPT-4の教師役と生徒役でペルソナ別の対話を大量自動生成
- **ソクラテスメソッドのプロンプト実装**: 「答えを教えない」をシステムプロンプトで制御
- **学習スタイルモデルを中間層として挟む設計**: 個々の性格→汎用的な学習タイプ→指導法、で転用可能に

### 結果
- 既存手法と比較してパーソナライズ精度・学習モチベーション向上で優位
- 個別ニーズを正確に識別し、学習体験を大幅改善

### 投稿ネタとしての切り口
- 「AIチューターは"全員同じ教え方"だった。PACEは学習スタイルを4軸で分類して個別指導する」
- 「GPT-4に先生役と生徒役をやらせて、対話データを自動生成する研究が面白い」
- 「ソクラテスメソッド×LLM：答えを教えないAI家庭教師が成績を上げた話」
- 「AI教育ツールを作るなら、Felder-Silverman学習スタイルモデルは覚えておくべき」
