...
1. `threads_automation/data/post-history.md` を読んで、metrics_fetched が false の投稿を特定する

2. Threads API でその投稿のデータを取得
   - エンドポイント: `https://graph.threads.net/v1.0/{投稿ID}/insights`
...
5. 取得結果のサマリーを表示
   - 各投稿の1行目 / views / likes / ER / 分類（バズ/好調/普通/低調）

## 注意
- 投稿から24時間以上経ったものだけ対象にする（数字が安定するまで待つ）
- API エラーの場合は metrics_fetched を false のままにして次回再取得
- 読者の「質問」コメントは次のアナリスト→ライターに繋がる大事なデータ

## 引数
$ARGUMENTS — "all" で全投稿を再取得。"account" でアカウント全体のインサイトも取得。
